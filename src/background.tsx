...
const DEEPL_API_KEY = process.env.REACT_APP_DEEPL_API_KEY || 'your_deepl_api_key_here';
...
async function translateText(text: string, settings: TranslationSettings): Promise<string> {
  ...
  const body = new URLSearchParams({
    ...
    ...(settings.fromLanguage !== 'auto' && { source_lang: settings.fromLanguage })
  });
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  if (areaName === 'sync' && changes.translationSettings) {
    console.log('Translation settings updated:', changes.translationSettings.newValue);
  }
});
